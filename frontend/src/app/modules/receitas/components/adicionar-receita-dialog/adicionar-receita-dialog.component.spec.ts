...
describe('AdicionarReceitaDialogComponent', () => {
  ...
  describe('Inicialização', () => {
    ...
    it('deve iniciar como inclusão', () => {
      ...
      expect(component.tituloDialog).toBe('Incluir receita');
    });
  ...
  describe('ngOnInit', () => {
    it('ngOnInit define data padrão no formato ISO quando não há receita', () => {
      component.ngOnInit();
  ...
  describe('Inicialização em edição', () => {
    ...
    it('deve iniciar como edição', () => {
      ...
      expect(component.tituloDialog).toBe('Editar receita');
    });

    it('deve preencher form com receita existente', () => {
      component.ngOnInit();
  ...
  describe('Categorias', () => {
    ...
    it('valueChanges de natureza inválida deve limpar categoria', () => {
      ...
      expect(component.form.get('categoria')?.value).toBe('Outra');
    });
  });
  ...
  describe('Salvar inclusão', () => {
    ...
    it('deve marcar form como touched quando inválido', () => {
      ...
      expect(receitasServiceMock.createReceita).not.toHaveBeenCalled();
    });

    it('deve criar receita com sucesso', () => {
      component.form.patchValue({
        pessoa: 'kelly',
        natureza: 'fixa',
        categoria: 'Casa',
      ...
      expect(receitasServiceMock.createReceita).toHaveBeenCalledWith({
        pessoa: 'Kelly',
        natureza: 'fixa',
        categoria: 'Casa',
        valor: 100,
    ...
    it('deve enviar data null quando data estiver vazia', () => {
      ...
      const payload = receitasServiceMock.createReceita.mock.calls[0][0];
      expect(payload.data).toBeNull();
    });

    it('deve tratar erro ao criar com error.error', () => {
      receitasServiceMock.createReceita.mockReturnValue(
    ...
    it('deve usar mensagem padrão quando erro não tiver mensagem', () => {
      receitasServiceMock.createReceita.mockReturnValue(throwError(() => ({})));
  ...
  describe('Salvar edição', () => {
    ...
    it('deve atualizar receitas com sucesso', () => {
      component.form.patchValue({
        pessoa: 'Kelly',
        natureza: 'fixa',
        categoria: 'Casa',
      ...
      const [id, payload] = receitasServiceMock.updateReceita.mock.calls[0];

      expect(id).toBe(mockReceita.id);
      expect(payload.pessoa).toBe('Kelly');
      expect(payload.valor).toBe(100);

      expect(dialogRefMock.close).toHaveBeenCalledWith(true);
    });

    it('deve tratar erro ao atualizar', () => {
      receitasServiceMock.updateReceita.mockReturnValue(
